  ...
  storyItems.forEach((item) => {
    ...
    const trigger = ScrollTrigger.create({
      ...
      onEnterBack: () =>
        gsap.to(item, { opacity: 1, duration: 0.4, ease: 'power2.out' }),
      onLeaveBack: () =>
        gsap.to(item, { opacity: 0.3, duration: 0.4, ease: 'power2.out' }),
    });
...
<template>
  ...
  >
    ...
    <!-- Cases -->
    <div v-for="caseItem in cases" :key="caseItem.index" class="pt-9 sm:pt-12">
      <!-- Step Indicator -->
      <div class="l-container pt-5 pb-9 lg:pb-12">
        <div
          class="section-d__step"
          :style="{
            backgroundImage: `url(${ASSETS_PATH}/img/intimate_relationships_p0402_heartsign_pcpadmob.svg)`,
          }"
        >
      ...
      <div
        class="flex flex-col gap-7 sm:gap-9 section-d__container m-auto mt-9 sm:mt-12 px-5 xs:px-6 sm:px-0"
      >
        <template v-for="(story, index) in caseItem.stories" :key="index">
          ...
          >
            <div
              ...
              :style="`max-width: ${story.width}px;`"
            >
              <div
